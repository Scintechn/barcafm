...
export function AdvertiserCard({
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-ink-100 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-brand-300 hover:shadow-lg">
      ...
      <p className="px-5 text-sm leading-relaxed text-ink-600">
        {advertiser.shortDescription}
      </p>

      {advertiser.promo && (
        <div className="mx-5 mt-4 flex items-start gap-2 rounded-xl bg-brand-50 px-3 py-2.5 text-xs">
          <Tag className="h-3.5 w-3.5 shrink-0 text-brand-700 mt-0.5" aria-hidden="true" />
      ...
      <div className="mt-auto flex flex-wrap items-center gap-x-4 gap-y-2 px-5 pb-5 pt-5 text-xs text-ink-600">
        {advertiser.phone && (
      ...
      <div className="flex items-center justify-between border-t border-ink-100 bg-ink-50/60 px-5 py-3 text-sm font-semibold text-brand-700 transition-colors group-hover:bg-brand-50">
